# -*- coding: utf-8 -*-
# ==============================================================================
# US State Department (2025—— ) Press Releases Crawler
#
# Description (English)
# ------------------------------------------------------------------------------
...
TOTAL_PAGES = 107  # 当前页数

OUT_CSV = "state_department_press_releases_2025_update.csv"
OUT_JSON = "state_department_press_releases_2025_update.json"
...
def parse_index_items(html: str) -> List[Dict]:
    """新版索引页结构: <li class="collection-result">"""
...
def parse_detail_content(html: str) -> str:
    ...
    for bad in entry.select(
        ...
        bad.decompose()
    for bad_sel in [
        ".post_tags", ".tags", ".sharethis-inline-share-buttons",
        ".report__back-to-top", ".social-share", ".wp-block-buttons"
    ]:
...
async def crawl_all():
    ...
    print(f"✅ 已完成 {len(done)} 条，开始爬取新版 {BASE} 到 {OUT_CSV} / {OUT_JSON}")
    ...
    async with async_playwright() as p:
        ...
        for i in tqdm(range(1, TOTAL_PAGES + 1), desc="索引页"):
            ...
            for it in items:
                ...
                try:
                    detail_html = await fetch_html(dpage, it["url"], "div.entry-content, article", 1.5)
